...
func buildTeamMD(team *store.TeamData, members []store.TeamMemberData, selfID uuid.UUID) string {
	...
	if selfRole == store.TeamRoleLead {
		...
		sb.WriteString("2. Release to worker: `team_tasks(action=\"release_to_worker\", text=\"enriched brief\", repo_key=\"repo-name\")`\n")
		sb.WriteString("   The `repo_key` param tells the worker which local repo to use (e.g. 'goclaw', 'frontend-app').\n")
		sb.WriteString("   If omitted, worker uses its default repo.\n")
